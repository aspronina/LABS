...
/* шаблон класс-контейнер в виде статической матрицы */
template <class T>
class matrix {
private:
	const int size = 2; // размер матрицы
	T mat[2][2]; // сама матрица
public:
...
int main() {
	...
	matrix<Lifting_transporting_machines> m; // объявляем матрицу
...
template <class T>
void matrix<T>::enter_matrix() // Изменение элементов матрицы
{
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			mat[i][j].set_machines();
		}
	}
}

template <class T>
void matrix<T>::get_matrix() const // Вывод элементов матрицы
{
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			cout << mat[i][j];
...
template <class T>
void matrix<T>::search() const // Поиск
{
	int lifting;
	cout << "ПОИСК\nВведите грузоподъемность: "; cin >> lifting;
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
			if (mat[i][j].get_lifting_power() == lifting) {
				cout << mat[i][j];
			}
		}
	}
...
template <class T>
void matrix<T>::compare() const // Сравнение
{
	int lifting;
	cout << "ПОИСК\nВведите грузоподъемность: "; cin >> lifting;
	for (int i = 0; i < size; i++) {
